"""
d2c-meets-fintech-reconciliation/backend/alerts.py
    ...
    async def send_sync_failure_alert(self, error: str) -> None:
        await self.send_alert(
            ...
            body=f"Nightly sync failed for your store. Error: {error}. "
                 "D2C meets fintech reconciliation team has been notified. Please check your API credentials.",
    ...
    async def _send_email(self, subject: str, body: str) -> None:
        ...
        try:
            ...
            resend.Emails.send({
                ...
                "subject": f"[D2C meets fintech reconciliation] {subject}",
                "html": f"<p>{body}</p><hr><small>D2C meets fintech reconciliation — Financial Reconciliation Engine</small>",
            })
                    ...
                    json={
                        "blocks": [
                            ...
                            },
                            {
                                "type": "context",
                                "elements": [
                                    {"type": "mrkdwn", "text": f"D2C meets fintech reconciliation • {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')}"}
                                ],
                            },
                        ]
                    },
            ...
            async with httpx.AsyncClient(timeout=10) as client:
                resp = await client.post(
                    ...
                    json={
                        "countryCode": "+91",
                        "phoneNumber": phone,
                        "callbackData": "d2c_meets_fintech_reconciliation_alert",
                        "type": "Template",
                        "template": {
                            "name": "d2c_meets_fintech_reconciliation_alert",
                            "languageCode": "en",
