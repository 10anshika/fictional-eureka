"""
d2c-meets-fintech-reconciliation/backend/tally.py
...
class ExportEngine:
    ...
    async def generate_export(
        ...
        try:
            ...
            if fmt == "tally":
                ...
                filename = f"d2c_meets_fintech_reconciliation_tally_{date_from}_{date_to}.csv"
            elif fmt == "zoho":
                csv_bytes = self._generate_zoho_csv(txns)
                filename = f"d2c_meets_fintech_reconciliation_zoho_{date_from}_{date_to}.csv"
            elif fmt == "quickbooks":
                csv_bytes = self._generate_quickbooks_csv(txns)
                filename = f"d2c_meets_fintech_reconciliation_qb_{date_from}_{date_to}.csv"
            else:
    ...
    def _upload_and_sign(self, filename: str, data: bytes) -> str:
        ...
        # Upload
        db.storage.from_("d2c-meets-fintech-reconciliation-exports").upload(
            path,
            data,
        ...
        signed = db.storage.from_("d2c-meets-fintech-reconciliation-exports").create_signed_url(
