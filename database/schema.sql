-- ============================================================
-- D2C MEETS FINTECH RECONCILIATION — MASTER DATABASE SCHEMA
...
CREATE TYPE audit_action AS ENUM (
  ...
  'connect_woocommerce', 'disconnect_woocommerce',
...
CREATE TABLE shopify_credentials (
  ...
  platform          ecom_platform NOT NULL DEFAULT 'shopify',
...
-- WooCommerce credentials (added for multi-platform support)
...
CREATE INDEX idx_woo_org ON woocommerce_credentials(org_id);
...
CREATE TABLE reconciled_transactions (
  ...
  -- Ecom data (field names kept for backward compat even when source is WooCommerce)
  ...
  -- Platform tagging
  ecom_platform         ecom_platform NOT NULL DEFAULT 'shopify',

  -- AI classification results (from /api/ai/classify internal tool)
...
CREATE INDEX idx_recon_platform ON reconciled_transactions(org_id, ecom_platform);
...
CREATE TABLE sync_logs (
  ...
  ecom_platform   ecom_platform,                     -- 'shopify' | 'woocommerce' (nullable for full_sync logs)
...
CREATE TABLE organization_settings (
  ...
  -- Multi-platform connection tracking
  connected_platforms   ecom_platform[] NOT NULL DEFAULT '{}',
  created_at            TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at            TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- woocommerce_credentials: CA blocked
...
BEGIN
  ...
  DELETE FROM woocommerce_credentials  WHERE org_id = p_org_id;
...
CREATE TRIGGER trg_woo_updated          BEFORE UPDATE ON woocommerce_credentials  FOR EACH ROW EXECUTE FUNCTION set_updated_at();
